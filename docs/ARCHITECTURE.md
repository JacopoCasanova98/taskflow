...
- **Refresh cookie:** successful register/login sets `TASKFLOW_REFRESH` with `HttpOnly=true`, `SameSite=Strict`, `Path=/api/auth`, no Domain, and Max-Age matching the refresh TTL. `Secure` uses the same centralized production-safe cookie setting as XSRF. JavaScript must never read refresh tokens. Clearing cookies must use matching scope when implemented in MS4.6.
...
MS4.5 implements only public `GET /api/auth/csrf`, `POST /api/auth/register`, and `POST /api/auth/login`; the public POSTs remain CSRF protected. All other `/api/**` routes retain Bearer authentication. `/refresh`, `/logout`, and `/me` below are future contracts, not implemented routes.
