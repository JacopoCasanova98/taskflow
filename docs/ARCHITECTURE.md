...
CSRF remains enabled. Unsafe requests without a valid CSRF token can receive `403` before authentication checks. The current default CSRF token repository may use a session for CSRF state, not a login session. MS4.4 preserves the existing unsafe-method CSRF matcher explicitly, overriding Resource Server's automatic Bearer exemption; Bearer GET requests are unaffected. Before exposing any public unsafe authentication endpoints in MS4.5, complete Spring Security SPA CSRF integration using `CookieCsrfTokenRepository` with Angular's `XSRF-TOKEN` / `X-XSRF-TOKEN` convention. No public auth exceptions or partial SPA flow are introduced in MS4.3–MS4.4.
